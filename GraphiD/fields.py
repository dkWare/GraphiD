...
class UIField:
    ...
    def raise_scroll_event(self, scroll_x: int, scroll_y: int):
        if not self._field_active: #field is not active
            return
        ...
        if self._mouse_pointer_in_field:
            self.on_mouse_scroll_on_field(_Vector2D(scroll_x, scroll_y, "raise_scroll_event_arg"))
    ...
    def on_mouse_scroll_on_field(self, mouse_scroll: _Vector2D):
        """
        This event gets called when the mouse hovers over this field and uses the scroll wheel

        Args:
            mouse_scroll (_Vector2D): Scroll Direction
        """
...
class ScrollBar:
    def __init__(self, id: str, *, center_x: float, center_y: float, width: float,
        ...
        self._scroll_group = scroll_group
        ...
        group_length = len(self._scroll_group.field_list) or 1
        total_height = group_length * self._first_field._height
        scroll_thumb_height = height / total_height
        scroll_thumb_height = 40

        thumb_point_a = _Vector2D.copy(body_point_b, "thumb_pointA")
        thumb_point_b = _Vector2D(body_point_b.valA+thumb_bar_width, body_point_b.valB, "thumb_pointB")
        thumb_point_c = _Vector2D(body_point_b.valA+thumb_bar_width, body_point_b.valB - scroll_thumb_height, "thumb_pointC")
        thumb_point_d = _Vector2D(thumb_point_a.valA, thumb_point_a.valB - scroll_thumb_height, "thumb_pointD")
...
class _scroll_bar_body(UIField):
    def __init__(self, id: str = None, *, pointA: _Vector2D = None, pointB: _Vector2D = None, pointC: _Vector2D = None, pointD: _Vector2D = None, ui_manager: _UIManager = None, head: ScrollBar):
        ...
        self._head = head
        self.set_color_masked(arcade.color.BLUE_GRAY)
        self.set_color_unmasked(arcade.color.BLUE_SAPPHIRE)

    def on_update(self):
        for field in self._head._scroll_group.field_list:
            if field._point_list[0].valB > self._point_list[0].valB:
                field.set_active(False)
                continue
            if field._point_list[2].valB < self._point_list[2].valB:
                field.set_active(False)
                continue
...
class _scroll_bar_thumb(UIField):
    ...
    def on_mouse_scroll_on_field(self, mouse_scroll: _Vector2D):
        if self._point_list[0].valB+mouse_scroll.valB > self._head._body._point_list[0].valB:
            return
        if self._point_list[2].valB+mouse_scroll.valB < self._head._body._point_list[2].valB:
            return
        self.change_position_rel(dy=mouse_scroll.valB)
        self._head._scroll_group.change_position_rel(dy=mouse_scroll.valB)